...
class SoftBinary(torch.nn.Module):
    def __init__(self, cin, dilate_size=7) -> None:
        '''
            ADM, Apply Softbinary to the binary mask
            fc is the attention layer
...
class DeformableAlign(torch.nn.Module):
    '''
        PAM Layer
        deform_conv for 可变性 Convolution
    ...
    def forward(self, features, bmasks, Ft2):
        '''
            offset contains 光流先验, used for multi-layer bias prediction
        '''
        outs = []
...
class Network(torch.torch.nn.Module):
    ...
    def get_cond(self, inps: list, time: float = 0.5):
        '''
            ASB
            FeaturePyramid: Multi-scale feature extraction
            PAM Align: DeformableAlign -- align feaR
            ADM: AttentionMerge -- Softbinary mask mixture
        '''
